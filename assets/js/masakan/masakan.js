$(document).ready(() => {
	...
	let table = $("table").DataTable({
		...
		columnDefs: [
			{
				render: (data) =>
					`<img class="w-100" src="${base_url}/uploads/${data}">`,
				targets: 0,
			},
			{
				orderable: false,
		...
		$.ajax({
			...
			error: () =>
				$(this)
					.find(".info")
					.html(
						`<div class="alert alert-danger">Format File Tidak Didukung</div>`
					),
		});
	});
	$("tbody").on("click", ".edit", function () {
		...
		$(".modal")
			.find("[name=id_kategori]")
			.append(`<option value="${data.id_kategori}">${data.kategori}</option>`);
